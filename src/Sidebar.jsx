...
function Sidebar() {
  ...
  const getAllThreads = async () => {
    try {
      const response = await fetch("/api/thread/"); // as we have defined in backend if any req comes on this route send all threads form db.
  ...
  const changeThread = async (newThreadId) => {
    ...
    try {
      const response = await fetch(`/api/thread/${newThreadId}/`); // Send the request to backend and the route defn will handel the rest
      const res = await response.json();
  ...
  const deleteThread = async (threadId) => {
    try {
      const response = await fetch(`/api/thread/${threadId}/`, {method: "DELETE"}); // We will send the delete req to th path and it will perform the task
      const res = await response.json();
      console.log(res);
